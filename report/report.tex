...
\midrule
\texttt{idx\_facility\_slots\_} \texttt{facility} & \texttt{Facility\_Slots} \texttt{(facility\_id)} & Availability lookup is always filtered by facility. \\
\texttt{idx\_bookings\_date\_} \texttt{status} & \texttt{Bookings} \texttt{(booking\_date, status)} & Admin utilization report and availability checks scan by date+status. \\
\texttt{idx\_booking\_slots\_slot} & \texttt{Booking\_Slots} \texttt{(slot\_id)} & Double-booking trigger queries by \texttt{slot\_id}; without this, every insert scans the table. \\
\texttt{idx\_wallet\_tx\_user\_} \texttt{time} & \texttt{Wallet\_Transactions} \texttt{(user\_id,} \texttt{created\_at DESC)} & "My Transactions" page sorts a user's ledger newest-first. \\
\texttt{idx\_facility\_rooms\_} \texttt{facility} & \texttt{Facility\_Rooms} \texttt{(facility\_id)} & Room dropdown listing for hall guest rooms / visitor hostel. \\
\bottomrule
\end{tabularx}
