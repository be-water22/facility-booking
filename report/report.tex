...
\pagestyle{fancy}
\fancyhf{}
\rhead{Campus Facility Booking System}
...
\title{
    \vspace{-1cm}
    \textbf{\Huge Campus Facility Booking System}\\[0.3cm]
    \Large CS315: Database Management Systems \\[0.1cm]
    \large Course Project Report
}
\author{
    Abhishek Meena (220048) \\
    Rythm Kumar (220925) \\
    Shubhanshu Mishra (221048) \\
    Subham Anand (221093)
}
\date{}
...
\midrule
Backend         & Python 3.10+, FastAPI, psycopg2 (raw SQL) \\
...
The deliberate choice of \textbf{raw SQL} is to keep the database logic visible at the application boundary and to allow direct demonstration of SQL features such as triggers, \texttt{CHECK} constraints, advisory locks, partial unique indexes, and explicit transaction control. Every query the application issues is the same query a database administrator would write, with no abstraction layer translating or hiding it. This makes the schema, the indexes, and the concurrency primitives the centerpiece of the project, which is the focus a DBMS course report demands.
...
\begin{tabularx}{\textwidth}{@{}>{\scriptsize\ttfamily\raggedright\arraybackslash}p{4.4cm}>{\scriptsize\ttfamily\raggedright\arraybackslash}p{5.4cm}>{\small\raggedright\arraybackslash}X@{}}
\toprule
{\normalfont\textbf{Index}} & {\normalfont\textbf{Table}} & {\normalfont\textbf{Why}} \\
\midrule
idx\_facility\_slots\_facility & Facility\_Slots(facility\_id) & Availability lookup is always filtered by facility. \\
idx\_bookings\_date\_status & Bookings(booking\_date, status) & Admin utilization report and availability checks scan by date+status. \\
idx\_booking\_slots\_slot & Booking\_Slots(slot\_id) & Double-booking trigger queries by slot\_id; without this, every insert scans the table. \\
idx\_wallet\_tx\_user\_time & Wallet\_Transactions(user\_id, created\_at DESC) & "My Transactions" page sorts a user's ledger newest-first. \\
idx\_facility\_rooms\_facility & Facility\_Rooms(facility\_id) & Room dropdown listing for hall guest rooms / visitor hostel. \\
\bottomrule
\end{tabularx}
...
{\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{longtable}{@{}lp{6cm}p{8.5cm}@{}}
\toprule
...
9.3  & \texttt{base\_type\_check}                    & \texttt{base\_type IN} \texttt{('Student','Faculty',} \texttt{'Admin','Organization')}. \\
...
9.6  & \texttt{facility\_slot\_endtime\_} \texttt{after\_start} & \texttt{end\_time > start\_time}. \\
9.7  & \texttt{facility\_slot\_unique}               & \texttt{UNIQUE(facility\_id,} \texttt{start\_time, end\_time)}. \\
9.8  & \texttt{double\_booking\_trigger}             & \texttt{prevent\_double\_booking} fires on duplicate slot+date. \\
9.9  & \texttt{ground\_booking\_trigger\_} \texttt{blocks\_non\_org} & Trigger blocks a Student from booking OAT. \\
9.10 & \texttt{org\_CAN\_book\_OAT}                  & Counter-test: Organization succeeds where Student failed. \\
9.11 & \texttt{delete\_user\_with\_bookings\_} \texttt{is\_restricted} & \texttt{ON DELETE RESTRICT} on \texttt{Bookings.user\_id}. \\
\bottomrule
...
\vspace{0.3cm}
\centering
\textit{Source code: \href{https://github.com/be-water22/facility-booking}{github.com/be-water22/facility-booking}}
