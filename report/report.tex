...
\midrule
\texttt{idx\_facility\_slots\_facility} & \texttt{Facility\_Slots} \texttt{(facility\_id)} & Availability lookup is always filtered by facility. \\
\texttt{idx\_bookings\_date\_status} & \texttt{Bookings} \texttt{(booking\_date, status)} & Admin utilization report and availability checks scan by date+status. \\
...
\texttt{idx\_wallet\_tx\_user\_time} & \texttt{Wallet\_Transactions} \texttt{(user\_id, created\_at DESC)} & "My Transactions" page sorts a user's ledger newest-first. \\
\texttt{idx\_facility\_rooms\_facility} & \texttt{Facility\_Rooms} \texttt{(facility\_id)} & Room dropdown listing for hall guest rooms / visitor hostel. \\
\bottomrule
\end{tabularx}
